...
import (
	...
	"github.com/wo4zhuzi/swaptool/config"
	"github.com/wo4zhuzi/swaptool/utils"
)
...
func Calc(amountB *big.Int, slippage float64) *big.Int {
...
func MultCalc(amountB *big.Float, decimals int) string {
	mul := decimal.NewFromFloat(float64(10)).Pow(decimal.NewFromFloat(float64(decimals)))
	num, _ := decimal.NewFromString(amountB.String())
	return num.Mul(mul).String()
	//min := new(big.Float).Mul(amountB, big.NewFloat(math.Pow(10.0, float64(decimals))))
	//return realAmountB.Text('f', decimals)

}
func NewTokenBasic(config *config.SwapConfig, address string, client *ethclient.Client) *TokenBasic {
	return &TokenBasic{
		Client: client,
		Wallet: address,
	}
...
func (c *TokenBasic) AllowanceStatus(tokenAddress, contractAddress string, amount *big.Int) (bool, error) {
	wallet := c.Wallet
